...
class Player5:
    ...
    def getAllowedMoves(self, currentBoard, currentBlockStatus, prevMove):
        ...
        for allowedBlock in self.checkAllowedBlocks(prevMove, currentBlockStatus):
            moveList += [
                (3 * allowedBlock[0] + move[0], 3 * allowedBlock[1] + move[1])
                for move in self.checkAllowedMarkers(
                    currentBoard[allowedBlock[0]][allowedBlock[1]]
                )
            ]
        return moveList
    ...
    def move(self, currentBoard, currentBlockStatus, oldMove, flag):
        ...
        formattedBoard = [
            [[[0] * 3 for i in range(3)] for j in range(3)] for j in range(3)
        ]
        ...
        if oldMove[0] < 0 or oldMove[1] < 0:
            ...
            uselessScore, nextMove = self.alphaBetaPruning(
                formattedBoard,
                formattedBlockStatus,
                -100000000,
                100000000,
                True,
                oldMove,
    ...
    def terminalCheck(self, currentBoard, currentBlockStatus):
        terminalStat = 0
        for i in range(3):
            if currentBlockStatus[i][0] == currentBlockStatus[i][
                1
            ] == currentBlockStatus[i][2] and currentBlockStatus[i][0] in (1, 2):
                terminalStat = currentBlockStatus[i][0]
            if currentBlockStatus[0][i] == currentBlockStatus[1][
                i
            ] == currentBlockStatus[2][i] and currentBlockStatus[0][i] in (1, 2):
                terminalStat = currentBlockStatus[0][i]
        if currentBlockStatus[0][0] == currentBlockStatus[1][1] == currentBlockStatus[
            2
        ][2] and currentBlockStatus[1][1] in (1, 2):
            terminalStat = currentBlockStatus[1][1]
        if currentBlockStatus[0][2] == currentBlockStatus[1][1] == currentBlockStatus[
            2
        ][0] and currentBlockStatus[1][1] in (1, 2):
            terminalStat = currentBlockStatus[1][1]
        if terminalStat == 0:
            for i in range(3):
                for j in range(3):
    ...
    def alphaBetaPruning(
        self, currentBoard, currentBlockStatus, alpha, beta, flag, prevMove, depth
    ):
        ...
        if flag:
            ...
            for move in possibMoves:
                ...
                tempBlockStatus[move[0] // 3][move[1] // 3] = self.getBlockStatus(
                    tempBoard[move[0] // 3][move[1] // 3]
                )

                childScore, childBest = self.alphaBetaPruning(
                    tempBoard, tempBlockStatus, alpha, beta, not flag, move, depth - 1
                )
                if childScore >= v:
                    v = childScore
                    bestMove = move
