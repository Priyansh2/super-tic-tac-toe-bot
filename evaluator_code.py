...
def update_lists(game_board, block_stat, move_ret, fl):
    ...
    if block_stat[block_no] == '-':
        if (
            game_board[id1 * 3][id2 * 3] == game_board[id1 * 3 + 1][id2 * 3 + 1]
            and game_board[id1 * 3 + 1][id2 * 3 + 1]
            == game_board[id1 * 3 + 2][id2 * 3 + 2]
            and game_board[id1 * 3 + 1][id2 * 3 + 1] != '-'
            and game_board[id1 * 3 + 1][id2 * 3 + 1] != 'D'
        ):
            mflg = 1
        if (
            game_board[id1 * 3 + 2][id2 * 3] == game_board[id1 * 3 + 1][id2 * 3 + 1]
            and game_board[id1 * 3 + 1][id2 * 3 + 1] == game_board[id1 * 3][id2 * 3 + 2]
            and game_board[id1 * 3 + 1][id2 * 3 + 1] != '-'
            and game_board[id1 * 3 + 1][id2 * 3 + 1] != 'D'
        ):
            mflg = 1
        if mflg != 1:
            for i in range(id2 * 3, id2 * 3 + 3):
                if (
                    game_board[id1 * 3][i] == game_board[id1 * 3 + 1][i]
                    and game_board[id1 * 3 + 1][i] == game_board[id1 * 3 + 2][i]
                    and game_board[id1 * 3][i] != '-'
                    and game_board[id1 * 3][i] != 'D'
                ):
                    mflg = 1
                    break
        if mflg != 1:
            for i in range(id1 * 3, id1 * 3 + 3):
                if (
                    game_board[i][id2 * 3] == game_board[i][id2 * 3 + 1]
                    and game_board[i][id2 * 3 + 1] == game_board[i][id2 * 3 + 2]
                    and game_board[i][id2 * 3] != '-'
                    and game_board[i][id2 * 3] != 'D'
                ):
                    mflg = 1
                    break
    if flag == 0:
...
def terminal_state_reached(game_board, block_stat, point1, point2):
    ...
    ## Row win
    if (
        (bs[0] == bs[1] and bs[1] == bs[2] and bs[1] != '-' and bs[1] != 'D')
        or (bs[3] != '-' and bs[3] != 'D' and bs[3] == bs[4] and bs[4] == bs[5])
        or (bs[6] != 'D' and bs[6] != '-' and bs[6] == bs[7] and bs[7] == bs[8])
    ):
        return True, 'W'
    ## Col win
    elif (
        (bs[0] == bs[3] and bs[3] == bs[6] and bs[0] != '-' and bs[0] != 'D')
        or (bs[1] == bs[4] and bs[4] == bs[7] and bs[4] != '-' and bs[4] != 'D')
        or (bs[2] == bs[5] and bs[5] == bs[8] and bs[5] != '-' and bs[5] != 'D')
    ):
        return True, 'W'
    ## Diag win
...
def simulate(obj1, obj2):
    ...
    while 1:  # Main game loop
        ...
        t2 = time.time()
        if t2 - t1 > TIMEALLOWED:
            WINNER, MESSAGE = decide_winner_and_get_message('P1', 'L', 'TIMED OUT')
            break

        # Check if list is tampered.
        if not (
            verification_fails_board(game_board, temp_board_state)
            and verification_fails_block(block_stat, temp_block_stat)
        ):
            WINNER, MESSAGE = decide_winner_and_get_message(
                'P1', 'L', 'MODIFIED CONTENTS OF LISTS'
            )
            break
        ...
        gamestatus, mesg = terminal_state_reached(
            game_board, block_stat, p1_pts, p2_pts
        )
        if gamestatus == True:
            print_lists(game_board, block_stat)
    ...
    print(WINNER)  ##one who wins
    print(string)  ##determine who is P1 ('X'),P2 ('O')
    print(
        MESSAGE
    )  ## Game message (TIMED OUT -> loosing player timeout, COMPLETE --> clear win, MORE BLOCKS --> when player win becoz he won more sub 3X3 grid, DRAW --> draw game)
