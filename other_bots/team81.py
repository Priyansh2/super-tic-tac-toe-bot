...
class Player81:
    ...
    def move(self, board, block, old_move, flag):
        ...
        move_utility = 0
        (new_move, move_utility) = self.search(
            newboard, block, new_old_move, flag, depth, alpha, beta
        )
        ...
        return block_cell_to_row_column(new_move[0], new_move[1])
    ...
    def search(self, board, block, old_move, flag, depth, alpha, beta):
        ...
        WIN_STAT = check_win_board(
            block, self.__flag
        )  # 1 -> win, -1 -> lose, 0 -> don't know
        if WIN_STAT == 1:
        ...
        valid_cells = determine_valid_cells(
            board, valid_blocks, block
        )  # valid_cells is a list block-cell format
        # print(valid_cells)
        ...
        if m_node == 0:  # if depth < SEARCH_DEPTH:
            ...
            for move in valid_cells:
                ...
                (tmove, move_utility) = self.search(
                    board, block, move, oflag, depth + 1, node_utility, alpha
                )
                board[move[0]][move[1]] = '-'
            ...
            for move in valid_cells:
                board[move[0]][move[1]] = flag
                (tmove, move_utility) = self.search(
                    board, block, move, oflag, depth + 1, beta, node_utility
                )
                board[move[0]][move[1]] = '-'
...
def check_fp(block, flag):
    ...
    for t1 in (C_TOPR, C_MIDDLER):
        for t2 in (C_LEFTC, C_MIDDLEC):
            if (
                check_tuple(block, t1, D_LT[flag]) or check_tuple(block, t1, D_LR[flag])
            ) and (
                check_tuple(block, t2, D_LT[flag]) or check_tuple(block, t2, D_LR[flag])
            ):
                MU += F_RC
                break
    ...
    for t1 in (C_TOPR, C_MIDDLER):
        for t2 in (C_RIGHTC, C_MIDDLEC):
            if (
                check_tuple(block, t1, D_RB[flag]) or check_tuple(block, t1, D_LR[flag])
            ) and (
                check_tuple(block, t2, D_LT[flag]) or check_tuple(block, t2, D_LR[flag])
            ):
                MU += F_RC
    ...
    for t1 in (C_BOTTOMR, C_MIDDLER):
        for t2 in (C_LEFTC, C_MIDDLEC):
            if (
                ...
                check_tuple(block, t2, D_RB[flag]) or check_tuple(block, t2, D_LR[flag])
            ):
                MU += F_RC
                break
    ...
    if (
        check_tuple(block, C_DIAGT, D_LT[flag])
        or check_tuple(block, C_DIAGT, D_RB[flag])
    ) and (
        check_tuple(block, C_DIAGB, D_LT[flag])
        or check_tuple(block, C_DIAGB, D_RB[flag])
    ):
        MU += F_DIAG
